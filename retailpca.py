...
#partition222=pd.read_csv('/Users/huyuxuan/desktop/222.csv')
...
#partition222[agecolnames].iloc[:,10:]
...
#agecolnames=vertical.loc[vertical.subcategory=='Age','colName'].tolist()


# In[9]:

#vertical.loc[vertical.colName=='F10579',]
...
#my_plot.legend(["Group0","Group2","Group3","Group4",'Group5','Group6'], loc=9,ncol=7)
sns.set(font_scale=3)
...
vertical.loc[vertical['colName'].isin(columns26)]
...
meank7df.to_csv('/Users/huyuxuan/desktop/finalproject/meank7df.csv',index=False)


# In[90]:

#this is all about MOR Bank
#Mail Order Responder, the number of times the kind of products purchase
#buying purchase--5,4,1,3,2,0,6
...
len(meank7df.columns[26:35])


# In[233]:

meank7df.columns[26:65]
...
print(sorted(columnstype2))


# In[273]:

print(sorted(columnstype3))


# In[274]:

#plt.plot(meank7df.label, meank7df.F794,alpha=0.8)


# In[213]:

#cluster analysis 3: cluster 2 is likely to buy car


# In[217]:

#cluster analysis 4: cluster 3 is likely to buy car
...
for i in columns39:
    plt.plot(meank7df.label, meank7df[i],alpha=0.8)
plt.show()
##auto in the market or buy auto--prediction


# In[88]:

meank7dftest6=meank7df.iloc[:,0:6]
...
meank7dftest6.plot(kind='density', subplots=True, layout=(3,3), sharex=False)


# In[306]:

partition20.iloc[1:10,:]
...
obser=partition20.groupby('label').count().latitude
...
lat=partition20.groupby('label').mean().latitude


# In[317]:

long=partition20.groupby('label').mean().longitude
...
m.save(os.path.join('/Users/huyuxuan/desktop', 'Features_3a.html'))


# In[342]:

min(partition20.latitude)


# In[343]:

max(partition20.latitude)


# In[344]:

min(partition20.longitude)


# In[345]:

max(partition20.longitude)


# In[347]:

std(partition20.longitude)


# In[ ]:

geoplotlib.dot(mapdf,color=)
